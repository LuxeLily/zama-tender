...
export const useWagmiEthers = (initialMockChains?: Readonly<Record<number, string>>) => {
  ...
  const providersResult = useMemo(() => {
    ...
    return {
      ethersProvider: new ethers.BrowserProvider(eip1193Provider),
      eip1193Provider,
    };
  }, [walletClient]);

  const providers = providersResult;
  const ethersProvider = providers?.ethersProvider;
  const eip1193Provider = providers?.eip1193Provider;
