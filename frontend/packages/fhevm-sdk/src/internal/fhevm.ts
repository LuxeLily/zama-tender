...
async function getChainId(
  providerOrUrl: Eip1193Provider | string | any
): Promise<number> {
  ...
  if (providerOrUrl && typeof providerOrUrl.getNetwork === "function") {
    const network = await providerOrUrl.getNetwork();
    return Number(network.chainId);
  }

  throw new Error("Provider does not support .request() or .getNetwork()");
}
...
export const createFhevmInstance = async (parameters: {
  ...
  // Resolve chainId
  console.log("[createFhevmInstance] resolving provider...");
  const { isMock, rpcUrl, chainId } = await resolve(providerOrUrl, mockChains);
  console.log(`[createFhevmInstance] resolved: isMock=${isMock}, chainId=${chainId}`);
  ...
  if (!isFhevmWindowType(window, console.log)) {
    console.log("[createFhevmInstance] SDK not on window, loading...");
    notify("sdk-loading");

    // throws an error if failed
    await fhevmLoadSDK();
    console.log("[createFhevmInstance] SDK loaded from CDN");
    throwIfAborted();

    notify("sdk-loaded");
  } else {
    console.log("[createFhevmInstance] SDK already on window");
  }

  // notify that state === "sdk-loaded"

  if (!isFhevmInitialized()) {
    console.log("[createFhevmInstance] SDK not initialized, initializing with thread: 0...");
    notify("sdk-initializing");

    // throws an error if failed
    await fhevmInitSDK({ thread: 0 });
  ...
  console.log("[createFhevmInstance] fetching public key from storage...");
  const pub = await publicKeyStorageGet(aclAddress);
  console.log("[createFhevmInstance] public key fetched");
  throwIfAborted();
  ...
  console.log("[createFhevmInstance] instance created!");

  // Save the key even if aborted
  console.log("[createFhevmInstance] saving keys to local storage...");
  await publicKeyStorageSet(
    aclAddress,
    instance.getPublicKey(),
    instance.getPublicParams(2048)
  );
  console.log("[createFhevmInstance] keys saved");
